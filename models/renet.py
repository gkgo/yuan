...
class RENet(nn.Module):
    ...
    def cca(self, spt, qry):
        ...
        spt = self.normalize_feature(spt)  # 10，640，5，5
        qry = self.normalize_feature(qry)  # 10，640，5，5
        ...
        # applying attention
        spt_attended = attn_s.unsqueeze(2) * spt.unsqueeze(0)  # （5，10，640，5，5）
        qry_attended = attn_q.unsqueeze(2) * qry.unsqueeze(1)  # （5，10，640，5，5）

        # averaging embeddings for k > 1 shots
        if self.args.shot > 1:
            spt_attended = spt_attended.view(num_qry, self.args.shot, self.args.way, *spt_attended.shape[2:])  # （10）5，2，5，640，5，5
            qry_attended = qry_attended.view(num_qry, self.args.shot, self.args.way, *qry_attended.shape[2:])  # （10）5，2，5，640，5，5
            spt_attended = spt_attended.mean(dim=1)  # （10）5，5，640，5，5
            qry_attended = qry_attended.mean(dim=1)  # （10）5，5，640，5，5
        ...
        qry_attended_pooled = qry_attended.mean(dim=[-1, -2])  # （10）5，5，640
